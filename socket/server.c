...
void *handle_client(void *arg) {
    ...
    while (1) {
        ...
        // //插入系统信息sql语句
        // char *system_info_sql;
        // double mem_pre =  system_info.used_mem /system_info.total_mem;
        // sprintf(system_info_sql, "insert into `SYSTEM_INFO` values ( '%s', %llf,%llf,%ld,'1','test');",inet_ntoa(sin_addr), system_info.cpu_usage,  mem_pre,system_info.timestamp );
        ...
        // //插入内存信息sql语句
        // char *mem_info_sql[100];
        // sprintf(mem_info_sql, "insert into `MEM_STATE` values ('%s', %llu,%llu,%llu,%llf,%s);",inet_ntoa(sin_addr), system_info.total_mem, system_info.used_mem, system_info.free_mem,mem_pre,time_str1);
        ...
        //     //插入磁盘信息sql语句
        //     double disk_pre =   disk_infos[i].used_size / disk_infos[i].total_size;
        //     char *disk_info_sql;
        //     sprintf(disk_info_sql, "insert into `DISK_STATE` values ('%s', '%s', %llu, %llu, %llu, %llf,%s);",inet_ntoa(sin_addr), disk_infos[i].disk_name, disk_infos[i].total_size, disk_infos[i].used_size, disk_infos[i].free_size, disk_pre, time_str1);
...
void init_mysql(){
    ...
    conn_ptr = mysql_real_connect(conn_ptr, "10.5.150.102", "root", "123456", "monitorData", 3306, NULL, 0); //连接MySQL testdb数据库，端口为3306
    if (conn_ptr)
    {
